...
module.exports.initialize = function (config) {
  ...
  app.use(viewHelpers(program, app, config))
  ...
  var silentDomains = [ 'js', 'css', 'vendor', 'img' ]
  var skip = function (req, res) {
    return silentDomains.includes(req._parsedUrl.pathname.split('/')[1])
  }
  if (config.get('application').loggingMode) {
    app.use(logger({ format: 'dev', skip }))
  }
  ...
  app.use(expValidator({ customSanitizers: require('./custom_sanitizers') }))
  ...
  function requireAuthentication (req, res, next) {
    if (!res.locals.user) {
      req.session.redirectTo = req._parsedOriginalUrl.pathname
  ...
  app.use(function (err, req, res, next) {
    ...
    var errors_ = require('./errors')
    errors_.send500(res, err, 'server error')
  })
