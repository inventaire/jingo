...
Page.prototype.fetchMetadata = function () {
  return new Promise(function (resolve, reject) {
    ...
    gitmech.log(this.filename, this.revision, function (err, metadata) {
      ...
      addItemPictureFromUser(metadata)
      .then(() => resolve())
...
Page.prototype.fetchHistory = function () {
  return new Promise(function (resolve, reject) {
    ...
    gitmech.log(this.filename, 'HEAD', 30, function (err, history) {
      ...
      resolve(Promise.all(history.map(addItemPictureFromUser)))
    }.bind(this))
  }.bind(this))
}

function addItemPictureFromUser (item) {
  if (!(item && item.email)) return Promise.resolve(item)
  var userId = item.email.split('@')[0]
  return userPicture.get(userId)
  .then((url) => {
    item.picture = url
    return item
  })
  .catch((err) => {
    log.error(err, 'addItemPictureFromUser err')
    return item
  })
}
...
models.pages.findStringAsync = promisify(models.pages.findString)
models.repositories.refreshAsync = promisify(models.repositories.refresh)
